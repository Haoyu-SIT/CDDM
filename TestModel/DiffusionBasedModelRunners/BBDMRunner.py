...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def sample(self, net, batch, sample_path, stage='train'):
        ...
        with open(metrics_path, 'w') as log_file:
            log_file.write("Image Name, MSE, RMSE, MAE, 95 Percentile, 99 Percentile\n")
            ...
            for i in range(batch_size):
                ...
                rmse = math.sqrt(mse)  # 使用math.sqrt计算标量的平方根
                mae = torch.mean(torch.abs(x_cpu - sample_cpu)).item()  # MAE
                percentile_95 = torch.quantile(torch.abs(x_cpu - sample_cpu), 0.95).item()  # 95%百分位数
                percentile_99 = torch.quantile(torch.abs(x_cpu - sample_cpu), 0.99).item()  # 99%百分位数


                # Save NC files
                save_single_NC(sample_cpu, sample_path, f'sample_{x_name[i]}', variable_name="sample")
                ...
                log_file.write(f"{x_name[i]}, {mse}, {rmse}, {mae}, {percentile_95}, {percentile_99}\n")
                # print(f"Saved sample, condition, and ground truth for {x_name[i]}")
                # print(f"MSE: {mse}, RMSE: {rmse}, MAE: {mae}, 95% Percentile: {percentile_95}, 99% Percentile: {percentile_99}")
