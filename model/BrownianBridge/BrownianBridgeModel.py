...
from model.utils import extract, default, percentile_loss
...
class BrownianBridgeModel(nn.Module):
    def __init__(self, model_config):
        ...
        # UNet
        self.target_size = model_params.UNetParams.image_size
    ...
    def forward(self, x, y, context=None):
        ...
        b, c, h, w, device, target_size, = *x.shape, x.device, self.target_size
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        return self.p_losses(x, y, context, t)

    def p_losses(self, x0, y, context, t, noise=None):
        b, c, h, w = x0.shape
        assert h == self.target_size and w == self.target_size, f"Expected x0 shape to be [{b}, {c}, {self.target_size}, {self.target_size}], but got {x0.shape}"
        ...
        elif self.loss_type == 'l1+relative 0.4':
            epsilon  = 1
            bias_per_point = torch.abs(objective - objective_recon) / (torch.abs(objective) + epsilon)
            recloss = torch.mean(bias_per_point) + 0.4 * (objective - objective_recon).abs().mean()
        elif self.loss_type == 'l1+relative 0.5':
            epsilon  = 1
            bias_per_point = torch.abs(objective - objective_recon) / (torch.abs(objective) + epsilon)
            recloss = torch.mean(bias_per_point) + 0.5 * (objective - objective_recon).abs().mean()
        elif self.loss_type == 'l1+relative 0.6':
            epsilon  = 1
            bias_per_point = torch.abs(objective - objective_recon) / (torch.abs(objective) + epsilon)
            recloss = torch.mean(bias_per_point) + 0.6 * (objective - objective_recon).abs().mean()
        elif self.loss_type == 'l1+relative 0.7':
            epsilon  = 1
            bias_per_point = torch.abs(objective - objective_recon) / (torch.abs(objective) + epsilon)
            recloss =  torch.mean(bias_per_point) + 3 * (objective - objective_recon).abs().mean()
        elif self.loss_type == 'huber':
            abs_error = torch.abs(objective - objective_recon)
            quadratic = torch.clamp(objective, max=0.1)
            linear = abs_error - quadratic
            losses = 0.5 * torch.pow(quadratic, 2) + 0.1 * linear
            recloss = torch.mean(losses)
        elif self.loss_type == 'relative Bias Sum':
            bias = (objective - objective_recon).abs()
            recloss = np.sum(bias) / np.sum(np.abs(objective)) * 100
        elif self.loss_type == 'relative Bias':
            epsilon  = 1
            bias_per_point = torch.abs(objective - objective_recon) / (torch.abs(objective) + epsilon)
            recloss = torch.mean(bias_per_point)
        elif self.loss_type == 'crps_loss':
            y_true_np = objective.detach().cpu().numpy()
            y_pred_np = objective_recon.detach().cpu().numpy()
            crps = xs.crps_ensemble(y_true_np, y_pred_np)
            recloss = torch.tensor(crps, dtype=torch.float32, device=y_true_np.device)
        elif self.loss_type =='99percentile':
            recloss = percentile_loss(objective_recon, objective, percentile=99)
    ...
    def q_sample(self, x0, y, t, noise=None):
        ...
        if self.objective == 'grad':
            ...
            raise NotImplementedError()
        # 混合x0和y，作为起点
        mixed_image = 0.5 * x0 + 0.5 * y + self.max_var * noise

        x_t = (1. - m_t) * x0 + m_t * mixed_image + sigma_t * noise
    ...
    def predict_x0_from_objective(self, x_t, y, t, objective_recon):
        ...
        if self.objective == 'grad':
            ...
            sigma_t = torch.sqrt(var_t)
            x0_recon = (x_t - m_t * noise - sigma_t * objective_recon) / (1. - m_t)
        elif self.objective == 'ysubx':
            x0_recon = noise - objective_recon
        else:
            raise NotImplementedError
        return x0_recon
    ...
    def p_sample(self, x_t, y, context, i, clip_denoised=False):
        ...
        if self.steps[i] == 0:
            ...
            noise = torch.randn_like(x_t)
            x_tminus_mean = (1. - m_nt) * x0_recon + m_nt * noise + torch.sqrt((var_nt - sigma2_t) / var_t) * \
                            (x_t - (1. - m_t) * x0_recon - m_t * noise)
